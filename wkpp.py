...
def _kmeans_plusplus_w(
    ...
    center_id = np.random.choice(range(n_samples), size=n_samples, replace=False, p=prob)
    indices = np.full(n_clusters, -1, dtype=int)
    if sp.issparse(X):
        centers[0] = X[center_id[0]].toarray()
    else:
        centers[0] = X[center_id[0]]
    indices[0] = center_id[0]
